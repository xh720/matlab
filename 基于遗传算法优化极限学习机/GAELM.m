function [accuracy,T_train2,T_sim2,T_test2,T_sim02]=GAELM(bestX,P_train,T_train,hiddennum,P_test,T_test)
...
[LW,TF,TYPE] = elmtrain2(p_train,t_train,hiddennum,activate_model,0,IW1,IB1);
%%  创建模型
  
%% %网络训练数据输出
train_sim1 = elmpredict(p_train, IW1, IB1, LW, TF, TYPE);  %训练集输出
%% 网络测试输出
test_sim2 = elmpredict(p_test , IW1, IB1, LW, TF, TYPE);  %测试集输出
...
ms_ga = abs(T_sim02-T_test2)./T_test2;
%% %%打印出评价指标
disp( ' -----------------------训练集误差计算----------------------- ')
...
save GA.mat pre2_ga r2_ga mae_ga rmse_ga ms_ga T_test
