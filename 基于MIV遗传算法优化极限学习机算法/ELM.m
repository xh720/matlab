function  [T_train,T_sim1,T_test,T_sim2]=ELM(Input_train,Output_train,Input_test,Output_test,hiddennumber)
%% 训练&测试BP网络
%% 输入
% x：一个个体的初始权值和阈值
% P：训练样本输入
...
[IW, B, LW, TF, TYPE] = elmtrain(input_train, output_train, hiddennumber, activate_model, 0);

%% %网络训练数据输出
train_sim1 = elmpredict(input_train, IW, B, LW, TF, TYPE);  %训练集输出
%% 网络测试输出
test_sim2 = elmpredict(input_test , IW, B, LW, TF, TYPE);  %测试集输出
