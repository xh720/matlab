function [T_train3,T_sim3,T_test3,T_sim03]=GAELM(bestX,new_ptrain,T_train,hiddennum,new_ptest,T_test)
...
[output_train, ps_output] = mapminmax(T_train, 0, 1);
output_test = mapminmax('apply', T_test, ps_output);
...
[LW,TF,TYPE] = elmtrain2(new_ptrain,output_train,hiddennum,activate_model,0,IW1,IB1);
%%  创建模型
...
R1 = 1 - norm(T_train3 - T_sim3)^2 / norm(T_train3 - mean(T_train3))^2;  %mean求均值
R2 = 1 - norm(T_test3  - T_sim03)^2 / norm(T_test3  - mean(T_test3 ))^2;

%% 平均绝对误差MAE
MAE1=sum(abs(T_sim3 - T_train3)) ./ m ;
MAE2=sum(abs(T_sim03 - T_test3)) ./ n ;

% MAE1=mean(abs(T_sim01 - T_train2)) ./ m ;
% MAE2=mean(abs(T_sim02 - T_test2)) ./ n ;

%% %%均方根误差 RMSE
error1 = sqrt(sum((T_sim3 - T_train3).^2)./m);
error2 = sqrt(sum((T_sim03 - T_test3).^2)./n);


%% %%均方误差MSE
mse1 = sum((T_sim3 - T_train3).^2)./m;
mse2 = sum((T_sim03 - T_test3).^2)./n;
...
% 各评价因子 曲线图
figure(3)
plot(1: m, T_train3, 'r-*', 1: m, T_sim3, 'b-o', 'LineWidth', 1);
legend('真实值','预测值')
xlabel('样本编号')
ylabel('样本数据')
...
figure(4)
plot(1: n, T_test3, 'r-*', 1: n, T_sim03, 'b-o', 'LineWidth', 1)
legend('真实值', '预测值')
xlabel('预测样本')
ylabel('预测结果')
