...
%%  参数初始化设置
hiddennum = 55;                   % 隐藏层维度 BP神经网络中通常小于输入节点数大于输出节点数
...
%% %% MIV 算法
[MIV,new_ptrain,new_ptest]=MIV(p_train,t_train,p_test,IWbest,IBbest);

%% MIV筛选后进行粒子群优化
[IWbest1,IBbest1]=pso(hiddennum,new_ptrain,t_train);

%%  建立模型
[LW, TF] = elmtrain1(new_ptrain, t_train, 'sig', 0, IWbest1, IBbest1);

%%  仿真预测
t_sim1 = elmpredict(new_ptrain, IWbest1, IBbest1, LW, TF);
t_sim2 = elmpredict(new_ptest , IWbest1, IBbest1, LW, TF);
...
xlabel('预测样本')
ylabel('预测结果')
string = {'训练集预测结果对比'; ['(R^2=' num2str(R1) ' RMSE=' num2str(RMSE1) ')']};
title(string)
xlim([1, M])
grid
...
xlabel('预测样本')
ylabel('预测结果')
string = {'测试集预测结果对比'; ['(R^2=' num2str(R2) ' RMSE=' num2str(RMSE2) ')']};
title(string)
xlim([1, N])
grid
...
ms_mpso = abs(T_sim2-T_test)./T_test;
%% %%计算准确率  
...
save MIVPSO.mat pre2_mpso r2_mpso mae_mpso rmse_mpso ms_mpso T_test
