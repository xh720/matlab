function [MIV,new_ptrain,new_ptest]=MIV(p_train,t_train,p_test,IWbest,IBbest)
%%  创建模型
activate_model = 'sig';  % 激活函数
[LW, TF] = elmtrain(p_train, t_train, activate_model, 0,IWbest, IBbest);
%% 通过初始样本数据集X建立新的训练样本
...
for i=1:n
    bb=['result_in',num2str(i), '= elmpredict(p_increase',num2str(i),',IWbest, IBbest, LW, TF);']; %加
    eval(bb)
end
for i=1:n
    cc=['result_de',num2str(i), '= elmpredict(p_decrease',num2str(i),',IWbest, IBbest, LW, TF);']; %加
    eval(cc)
end
