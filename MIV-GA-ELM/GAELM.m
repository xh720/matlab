function [T_train1,T_sim1,T_test1,T_sim01]=GAELM(bestX,Input_train,Output_train,hiddennumber,Input_test,Output_test)
...
[LW,TF,TYPE] = elmtrain(input_train,output_train,hiddennumber,activate_model,0,IW1,IB1);
%%  创建模型
  
%% %网络训练数据输出
train_sim1 = elmpredict(input_train, IW1, IB1, LW, TF, TYPE);  %训练集输出
%% 网络测试输出
test_sim2 = elmpredict(input_test , IW1, IB1, LW, TF, TYPE);  %测试集输出
...
T_sim1=Train_sim1;
T_sim01=Test_sim2;
T_train1=Output_train;
T_test1=Output_test;

%% %%决定系数
% R1 = 1 - (sum((T_sim1 - T_train).^2)/ sum((T_sim1 - mean(T_train)).^2));
% R2 = 1 - (sum((T_sim2 - T_test).^2) / sum((T_sim2 - mean(T_test)).^2));
R1 = 1 - norm(T_train1 - T_sim1)^2 / norm(T_train1 - mean(T_train1))^2;  %mean求均值
R2 = 1 - norm(T_test1  - T_sim01)^2 / norm(T_test1  - mean(T_test1 ))^2;

%% 平均绝对误差MAE
MAE1=sum(abs(T_sim1 - T_train1)) ./ m ;
MAE2=sum(abs(T_sim01 - T_test1)) ./ n ;

% MAE1=mean(abs(T_sim01 - T_train2)) ./ m ;
% MAE2=mean(abs(T_sim02 - T_test2)) ./ n ;

%% 平均偏差MBE
mbe1 = sum(T_sim1 - T_train1) ./ m ;
mbe2 = sum(T_sim01 - T_test1 ) ./ n ;

%% %%均方根误差 RMSE
error1 = sqrt(sum((T_sim1 - T_train1).^2)./m);
error2 = sqrt(sum((T_sim01 - T_test1).^2)./n);


%% %%均方误差MSE
mse1 = sum((T_sim1 - T_train1).^2)./m;
mse2 = sum((T_sim01 - T_test1).^2)./n;
...
% 
%% 平均绝对百分比误差MAPE
MAPE1=mean(abs((T_train1 -T_sim1)./T_train1));
MAPE2=mean(abs((T_test1 -T_sim01)./T_test1));
...
figure(3)
plot(1: n, T_test1, 'r-*', 1: n, T_sim01, 'b-o', 'LineWidth', 1)
legend('真实值', '预测值')
xlabel('预测样本')
ylabel('预测结果')
