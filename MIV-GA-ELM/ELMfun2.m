function err=ELMfun2(x,new_ptrain,Output_train,hiddennumber,new_ptest,Output_test)
%% 训练&测试网络
%% 输入
% x：一个个体的初始权值和阈值
% P：训练样本输入
...
w2=x(1:w1num);   %初始输入层到隐层的权值
w2 = reshape(w2,hiddennumber,inputnum);
B2=x(w1num+1:w1num+hiddennumber);  %初始隐层阈值
B2=reshape(B2,hiddennumber,1);

%% ELM 训练
% 创建ELM网络
activate_model = 'sig';  % 激活函数

[LW,TF,TYPE] = elmtrain2(new_ptrain,output_train,hiddennumber,activate_model,0,w2,B2);
%%  创建模型
  
% ELM仿真测试
t_sim = elmpredict(new_ptest,w2,B2,LW,TF,TYPE);
% 反归一化
